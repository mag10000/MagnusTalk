extends CanvasLayer  
signal function_call(func_name)
var check_line
var lines
var current_line = 0
var last_line
...
func start_dialouge():
	...
	if not current_line > last_line:
		...
		if line_text.contains("Mcallfunc["):
			var pre1 = line_text.split("]")[0]
			var final = pre1.split("Mcallfunc[")[1]
			function_call.emit(final)
			current_line += 1
			start_dialouge()
			return
		else:
			...
			if line_text.contains("Mjump["):
				...
				if line_text.contains("Msound[") && use_audio == true:
					var path = line_text.replace(line_text.split("Msound[")[0],"").split("]")[0].replace("]","").replace("Msound[","")
